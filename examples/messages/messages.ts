...
async function demonstrateMessageFields(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Found ${standardMessages.data.length} messages with standard fields`
    );
    for (const message of standardMessages.data) {
      console.log(`- ${message.id}: ${message.subject || '(no subject)'}`);
      console.log(
        `  From: ${message.from?.map((f) => `${f.name} <${f.email}>`).join(', ') || 'Unknown'}`
      );
      console.log(
        `  Tracking Options: ${message.trackingOptions ? 'Present' : 'Not present'}`
      );
    ...
    console.log(
      `Found ${trackingMessages.data.length} messages with tracking options`
    );
    for (const message of trackingMessages.data) {
      ...
      if (message.trackingOptions) {
        ...
        console.log(
          `    Thread Replies: ${message.trackingOptions.threadReplies}`
        );
    ...
    for (const message of headerMessages.data) {
      ...
      if (message.headers && message.headers.length > 0) {
        ...
        message.headers.slice(0, 3).forEach((header) => {
          console.log(
            `    ${header.name}: ${header.value.substring(0, 100)}${header.value.length > 100 ? '...' : ''}`
          );
        });
    ...
    if (error instanceof NylasApiError) {
      ...
      console.error(
        `Unexpected error in demonstrateMessageFields: ${error.message}`
      );
    }
    throw error;
  }
...
async function demonstrateRawMime(): Promise<void> {
  ...
  try {
    ...
    if (rawMessage.data.rawMime) {
      console.log(
        `- Raw MIME Data: ${rawMessage.data.rawMime.substring(0, 100)}... (truncated)`
      );
      console.log(
        `- Raw MIME Length: ${rawMessage.data.rawMime.length} characters`
      );
      ...
      console.log(
        `- Body: ${rawMessage.data.body ? 'Present' : 'Not included'}`
      );
      console.log(
        `- From: ${rawMessage.data.from ? 'Present' : 'Not included'}`
      );
    } else {
...
async function demonstrateMessageSending(): Promise<NylasResponse<Message> | null> {
  ...
  try {
    ...
    if (!testEmail) {
      console.log(
        'TEST_EMAIL environment variable not set. Skipping message sending demo.'
      );
      console.log(
        'Set TEST_EMAIL=your-email@example.com to test message sending.'
      );
      return null;
    }
    ...
    console.log(
      `- To: ${sentMessage.data.to?.map((t) => `${t.name} <${t.email}>`).join(', ')}`
    );
    ...
    if (error instanceof NylasApiError) {
      ...
      console.error(
        `Unexpected error in demonstrateMessageSending: ${error.message}`
      );
    }
    return null;
  }
...
async function demonstratePlaintextMessageSending(): Promise<NylasResponse<Message> | null> {
  ...
  try {
    ...
    if (!testEmail) {
      console.log(
        'TEST_EMAIL environment variable not set. Skipping plaintext message sending demo.'
      );
      return null;
    }
    ...
    if (error instanceof NylasApiError) {
      ...
      console.error(
        `Unexpected error in demonstratePlaintextMessageSending: ${error.message}`
      );
    }
    return null;
  }
...
async function demonstrateMessageUpdate(messageId: string): Promise<void> {
  ...
  try {
    ...
    console.log(
      `- Metadata: ${JSON.stringify(updatedMessage.data.metadata, null, 2)}`
    );
  } catch (error) {
    if (error instanceof NylasApiError) {
      ...
      console.error(
        `Unexpected error in demonstrateMessageUpdate: ${error.message}`
      );
    }
    throw error;
  }
...
async function demonstrateScheduledMessages(): Promise<void> {
  ...
  try {
    ...
    if (schedules.length === 0) {
      console.log(
        "No scheduled messages found. This is normal if you haven't scheduled any messages."
      );
      return;
    }
    ...
    for (const schedule of schedules) {
      ...
      console.log(
        `  Status: ${schedule.status.code} - ${schedule.status.description}`
      );
      if (schedule.closeTime) {
        console.log(
          `  Close Time: ${new Date(schedule.closeTime * 1000).toISOString()}`
        );
      }
    }
    ...
    console.log(
      `- Status: ${scheduleDetails.data.status.code} - ${scheduleDetails.data.status.description}`
    );
  } catch (error) {
    if (error instanceof NylasApiError) {
      ...
      console.error(
        `Unexpected error in demonstrateScheduledMessages: ${error.message}`
      );
    }
    throw error;
  }
...
async function demonstrateMessageCleaning(): Promise<void> {
  ...
  try {
    ...
    for (const cleanedMessage of cleanedMessages.data) {
      ...
      console.log(
        `  Original body length: ${cleanedMessage.body?.length || 0} characters`
      );
      console.log(
        `  Cleaned conversation length: ${cleanedMessage.conversation?.length || 0} characters`
      );
      if (cleanedMessage.conversation) {
        const preview = cleanedMessage.conversation.substring(0, 100);
        console.log(
          `  Cleaned preview: ${preview}${cleanedMessage.conversation.length > 100 ? '...' : ''}`
        );
      }
    }
  } catch (error) {
    if (error instanceof NylasApiError) {
      ...
      console.error(
        `Unexpected error in demonstrateMessageCleaning: ${error.message}`
      );
    }
    throw error;
  }
...
async function demonstrateMessageQuerying(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Found ${attachmentMessages.data.length} messages with attachments`
    );
    ...
    if (error instanceof NylasApiError) {
      ...
      console.error(
        `Unexpected error in demonstrateMessageQuerying: ${error.message}`
      );
    }
    throw error;
  }
...
async function main(): Promise<void> {
  try {
    ...
    console.log(
      `API URI: ${process.env.NYLAS_API_URI || 'https://api.us.nylas.com'}`
    );
